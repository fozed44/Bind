...
(function(_bind){
    ...
    /*
        bind.Panel
            ...
            Panelizes any elements that have a .bind-panel css class.

        Remarks

            All elements that have the .bind-panel class will be 'panelized'. In other words, this means that a panel
    ...
    */
    _bind.Panel = new function Panel() {
        ...
        function throwGenericError(){
            throw new Error('bindPanel: unknown error!');
        }
        ...
        */
        function getServerResult(remoteKey){
            ...
            console.log("bindPanel: remoteKey '" + remoteKey + "' was not found in server results.");
            throwGenericError();
        }
        ...
        */
        function parseLocalItems(itemData){
            ...
            if(!items.length)
                return [];
            if(items.length & 1){
                console.log('bindPanel: Panel with invalid item data.');
                throwGenericError();
            }
            var result = [];
        ...
        */
        function readRemoteAddress(element){
            ...
            && _serverConfig.remoteAddress != remoteAddress)
                throw new Error('bindPanel: Multipe server addresses are not suppored.');
            _serverConfig.remoteAddress = remoteAddress;
        }
        ...
        */
        ...
        function createPanelContainer() {
            ...
            panelContainer.className = 'bind-panel-container';
            return panelContainer;
        } 

        function createPanel(config){            
            var panel = document.createElement('div');
            panel.className = 'bind-panel-panel';
        ...
        function createHeader(element, config){
            ...
            headerDiv.className = 'bind-panel-heading';
        ...
        function createDataItem(text){
            ...
            containerDiv.className = 'bind-panel-data-item';
        ...
        function addMouseHandlers(element, panel){
            _bind.Event.addHandler(element, 'mouseover', function(){
                element.appendChild(panel);
                panel.style.opacity = 0;
                _bind.Animate.createAnimator(panel, 'opacity', 1, 180, 30);
            });
            _bind.Event.addHandler(element, 'mouseleave', function(){
        ...
        function panelizeAllElements(){
            ...
            m('.bind-panel').each(function(serverLoadRequired){
        ...
        (function(){
            _bind.Event.addHandler(GlobalObject, 'onLoadComplete', function(){
                panelizeAllElements();
            });
        })();
